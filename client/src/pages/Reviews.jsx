...
export default function Reviews() {
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <section className="section grid gap-14 lg:grid-cols-[1.1fr_.9fr]"><div>
      ...
      {reviews.map((review) => <article className="mb-5 rounded-2xl border border-[#eaded8] bg-white p-6" key={review.id}><FaQuoteLeft className="text-blush text-2xl" /><Stars value={review.rating} /><h2 className="display mt-3 text-2xl">{review.title}</h2><p className="mt-3 leading-7 text-[#786869]">“{review.description}”</p><p className="mt-4 text-sm font-bold">{review.name} <span className="font-normal text-[#8d7b7a]">· {review.city}</span></p></article>)}
